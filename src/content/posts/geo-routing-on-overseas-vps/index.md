...
境内外分流的判定应该落在 IP 上，在一台不希望直连境内的服务器上这个理由尤其充分：要防的事情发生在 TCP 连接上，服务器用自己的 IP 连上京东的大陆地址，暴露就产生了，而连它的海外边缘节点不会（代理回国流量不安全这件事社区讨论过很多，也有[实测](https://github.com/net4people/bbs/issues/129#issuecomment-1308102504)），不过这 **只对跨境链路上的观测方成立**，站在服务提供方那一侧要另算，后面的[ECS部分](#设-ecs-防的是哪一种暴露)单独讲。
...
:::tip[在客户端建立完善的分流机制才是最佳实践]
上面的策略是在服务端上兜底，真正兼顾使用体验和安全的配置是在客户端上使用 `process` 进程名规则或分应用代理。而网传的某些利用“网络beacon”的钓 IP 问题，这里不做展开。
:::
