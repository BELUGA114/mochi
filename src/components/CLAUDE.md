...
- `control/WallpaperToggle.astro` is the homepage's wallpaper-immersive switch. It flips
  `body.wallpaper-immersive`, sets `inert` on the three hidden containers, swaps `aria-pressed` and the
  label, and drives the button's idle-hide state; every *hiding* rule lives in `Layout.astro`'s global style
  block. It must be rendered on every page and stay outside the Swup containers (both are load-bearing — see
